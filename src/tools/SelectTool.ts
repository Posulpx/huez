...
export class SelectTool implements Tool {
  ...
  onPointerDown(ctx: ToolContext): void {
    ...
    if (target) {
      ...
      if (target instanceof ArtboardElement && !target.hitLabel(ctx.point, ctx.renderer.scale)) {
        return;
      }
  ...
  private startMarquee(ctx: ToolContext): void {
    this.marquee = { x0: ctx.point.x, y0: ctx.point.y, x1: ctx.point.x, y1: ctx.point.y };
    this.marqueeBase = new Set(ctx.shiftKey ? ctx.scene.selected.map((s) => s.id) : []);
    if (!ctx.shiftKey) {
  ...
  private cloneSelection(ctx: ToolContext, offsetX: number, offsetY: number): void {
    ...
    for (const src of sources) {
      ...
      if (src instanceof ArtboardElement) {
        for (const child of ctx.scene.all) {
          if (child.artboardId === src.id && !(child instanceof ArtboardElement)) {
  ...
  private artboardUnderPoint(p: Point, scene: ToolContext["scene"]): ArtboardElement | null {
