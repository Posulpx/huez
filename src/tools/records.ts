...
export function ensureTool(id: string, label: string): ToolRecord {
  ...
  if (!r) {
    r = { id, label, active: false, used: 0, lastUsed: 0, shared: {}, specific: {} };
    records.set(id, r);
  }
  return r;
}

export function setToolActive(id: string, label: string, active: boolean): void {
...
export function recordToolProps(id: string, label: string, shared: PropertyBag, specific: PropertyBag): void {
  const r = ensureTool(id, label);
  r.shared = shared;
  r.specific = specific;
  r.lastUsed = Date.now();
...
export function elementProps(el: BaseElement): { shared: PropertyBag; specific: PropertyBag } {
  const s = el.style;
