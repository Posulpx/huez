...
export class PropertiesPanel {
  ...
  private buildCommon(el: BaseElement): void {
    ...
    this.root.appendChild(
      this.colorRow("Fill", s.fill ?? "#000000", (v) => {
        s.fill = v;
        this.requestRender();
      }, () => {
        s.fill = null;
    ...
    this.root.appendChild(
      this.colorRow("Stroke", s.stroke ?? "#000000", (v) => {
        s.stroke = v;
        this.requestRender();
      }, () => {
        s.stroke = null;
  ...
  private checkboxRow(label: string, value: boolean, onChange: (v: boolean) => void): HTMLElement {
    const row = document.createElement("div");
    row.className = "prop-row";
  ...
  private textRow(label: string, value: string, onInput: (v: string) => void): HTMLElement {
    const row = document.createElement("div");
    row.className = "prop-row";
  ...
  private nameRow(el: BaseElement): HTMLElement {
    ...
    input.type = "text";
    input.value = el.name;
    input.addEventListener("change", () => this.scene.rename(el, input.value || el.name));
