...
export function handlePoints(el: BaseElement): { id: HandleId; x: number; y: number }[] {
  const r = rectOf(el);
  return localHandlePoints(r).map((h) => ({ id: h.id, ...worldPointRect(r, h.x, h.y) }));
}
...
export function handleEdges(id: HandleId): { west: boolean; east: boolean; north: boolean; south: boolean } {
  return {
