...
export class Scene {
  ...
  get artboards(): readonly ArtboardElement[] {
    return this.elements.filter((e): e is ArtboardElement => e instanceof ArtboardElement);
  }
   ...
   */
  hitTest(p: Point): BaseElement | null {
    for (let i = this.elements.length - 1; i >= 0; i--) {
      const el = this.elements[i]!;
      if (!el.visible || el.locked) continue;
      if (el instanceof ArtboardElement) continue;
      if (el.hitTest(p)) return el;
    }
    for (let i = this.elements.length - 1; i >= 0; i--) {
      const el = this.elements[i]!;
      if (!el.visible || el.locked) continue;
      if (el instanceof ArtboardElement && el.hitTest(p)) return el;
    }
    return null;
  }
}

function sameOrder(a: readonly BaseElement[], b: readonly BaseElement[]): boolean {
